...
export default async function BlogPage({ params }: Props) {
  ...
  return (
    <>
      ...
      {blogList && (<div
        className='grid text-black bg-none mt-8 md:mt-16 container max-w-[2000px] m-auto 
      ...
      '>
        {blogList?.map((blog: { _id: string; title: string; content: string; date: string }, index: number) => {
          return (
            <Link key={blog._id} href={`/${lang}/blog/article/${blog._id}`} className='relative' prefetch={true}>
